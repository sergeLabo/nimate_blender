...
print("\nI am my_nimate.py in directory /nimate_blender/2.79b\n")
...
class OSC():
    ...
    def decodeOSC(data):
        table = { "i" : OSC.readInt, "f" : OSC.readFloat, "s" : OSC.readString,
                  "b" : OSC.readBlob, "d" : OSC.readDouble }
...
class NImateReceiver():
    ...
    def __init__(self, UDP_PORT, QUIT_PORT):
        ...
        print("NI mate Tools started listening to OSC on port " + str(UDP_PORT))

    def __del__(self):
        self.sock.close()
        print("NI mate Tools stopped listening to OSC")
...
class DelicodeNImateFeed():
    ...
    def __init__(self, image_name, sensor_num, feed_2):
        ...
        try:
            ...
            print("NI mate Tools Error: No texture with name: " + image_name)
            pass
        ...
        try:
            ...
            if feed_2:
                print("NI mate Tools replacing " + image_name + " with sensor " +\
                                        str(sensor_num) + " live feed 2")
            else:
                print("NI mate Tools replacing " + image_name + " with sensor " +\
                                        str(sensor_num) + " live feed 1")
        except Exception as e:
            print("NI mate Tools Error: Couldn't open NI mate feed " +\
...
if not GE:
    class DelicodeNImateFeedPlaneCreate(bpy.types.Operator):
        ...
        def execute(self, context):
            ...
            feed_prefix = "NImateFeed" + str(2*(context.scene.delicode_ni_mate_sensor-1) +\
                            (context.scene.delicode_ni_mate_feed == 'FEED2') + 1)

            plane.name = feed_prefix + "_Plane"

            if (feed_prefix + "_Image") not in bpy.data.images:
                bpy.ops.image.new(name=feed_prefix + "_Image", width=640,
                                    height=480, generated_type='UV_GRID')

            bpy.ops.mesh.uv_texture_add()
            plane.data.uv_textures[0].data[0].image = bpy.data.images[feed_prefix +\
                                                                        "_Image"]
    ...
    class DelicodeNImateReceiverLogicCreate(bpy.types.Operator):
        ...
        bl_label = "Create game logic"
        bl_description = "Create or update game logic for the receiver for the selected object"
        bl_options = {'REGISTER'}
    ...
    class DelicodeNImate(bpy.types.Operator):
        ...
        def execute(self, context):
            ...
            self.timer = context.window_manager.event_timer_add(1/context.scene.render.fps,
                                                                context.window)
    ...
    class VIEW3D_PT_DelicodeNImateGEPanel(bpy.types.Panel):
        ...
        def draw(self, context):
            ...
            if context.object != None and "CNImateReceiver" in context.object.game.controllers:
                col.operator("wm.delicode_ni_mate_receiver_logic_create",
                                    icon='FILE_REFRESH', text="Update game logic")
            else:
            ...
            layout.prop_search(scene, "delicode_ni_mate_feed_image", bpy.data,
                                                    "images", text="Or replace")
            col = layout.column()
            if context.object != None and "CNImateFeed" in context.object.game.controllers:
                col.operator("wm.delicode_ni_mate_feed_logic_create",
                                    icon='FILE_REFRESH', text="Update game logic")
            else:
    ...
    def init_properties():
        ...
        scene.delicode_ni_mate_port = bpy.props.IntProperty(
            name="Port",
            description="Receive OSC on this port, idem the Full Skeleton port in NI mate!",
            default = 7000,
            min = 0,
            max = 65535)
...
def updateGE(controller):
    ...
    bge.logic.DelicodeNImate.run(bge.logic.getCurrentScene().objects,
                                 set_GE_location,
                                 set_GE_rotation)
    # Actualisation de l'armature nécessaire
    bge.logic.getCurrentScene().objects.get(MY_RIG).update()
